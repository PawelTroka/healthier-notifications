...
### Reduce automated work email within work hours

Quiet hours control when work notifications arrive. They do not decide which emails deserve attention during the work window. Outlook's per-account **All**, **Focused** and **None** mail options are separate from Calendar notifications; Focused Inbox does not guarantee that an email is a direct human message or requires action. Keep personal-account choices separate. [Outlook notification controls](https://support.microsoft.com/en-US/Outlook/how-can-i-turn-push-notifications-and-sounds-on-or-off).

When a service sends both routine reports and useful requests, inspect its personal notification subscriptions before blocking its sender or Outlook's mixed mail category. For example, Azure DevOps exposes personal subscription opt-outs and build/PR filters. Build filters include status, requested by/for, definition and project; PR filters depend on the selected event template. Review explicit user/project criteria, preserve personally relevant review requests and mentions, and verify a replacement before removing a broad subscription. This is a separate account-settings review; the phone toolkit does not edit subscriptions or mailbox rules. [Personal subscriptions](https://learn.microsoft.com/en-us/azure/devops/organizations/notifications/manage-your-personal-notifications?view=azure-devops), [supported filter fields](https://learn.microsoft.com/en-us/azure/devops/organizations/notifications/oob-supported-event-types?view=azure-devops).
